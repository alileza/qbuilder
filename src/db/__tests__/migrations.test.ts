...
import { runMigrations, migrations, DEFAULT_TABLE_PREFIX } from '../migrations.js';
...
describe('Migrations', () => {
  ...
  describe('migrations array', () => {
    ...
    it('should have up and down SQL functions for each migration', () => {
      for (const migration of migrations) {
        expect(typeof migration.up).toBe('function');
        expect(typeof migration.down).toBe('function');
        // Generate SQL with default prefix
        const upSql = migration.up(DEFAULT_TABLE_PREFIX);
        const downSql = migration.down(DEFAULT_TABLE_PREFIX);
        expect(upSql.length).toBeGreaterThan(0);
        expect(downSql.length).toBeGreaterThan(0);
      }
    });

    it('should use the provided prefix in generated SQL', () => {
      const prefix = 'myapp_';
      const upSql = migrations[0].up(prefix);
      expect(upSql).toContain('myapp_questionnaires');
      expect(upSql).toContain('myapp_questionnaire_versions');
    });
  });
  ...
  describe('runMigrations', () => {
    ...
    it('should use custom table prefix when provided', async () => {
      mockPool.query
        .mockResolvedValueOnce({}) // Create migrations table
        .mockResolvedValueOnce({ rows: [] }) // Get applied migrations
        .mockResolvedValueOnce({}) // Run migration 1
      ...
      await runMigrations(mockPool as any, { tablePrefix: 'custom_' });

      // Should create custom_migrations table
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('CREATE TABLE IF NOT EXISTS custom_migrations')
      );
      // Should insert into custom_migrations
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO custom_migrations'),
        ['001_create_questionnaires']
      );
    });
  });
